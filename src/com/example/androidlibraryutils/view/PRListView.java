 ...
 * 
 */
public class PRListView extends ListView implements OnScrollListener,
		OnClickListener {
	// 拖拉ListView枚举所有状态
	...
	public void initDragListView(Context context) {
		...
		time = dateFormat.format(new Date(System.currentTimeMillis()));// 更新时间
	...
	public void initHeadView(Context context, String time) {
		...
		// 显示更新事件
		mLastUpdateTextView.setText("更新于:" + time);
	...
	public boolean onTouchEvent(MotionEvent ev) {
		if (isAutoRefresh) {
			mLastUpdateTextView.setText("更新于:" + dateFormat.format(new Date(System.currentTimeMillis())));// 更新时间
			switch (ev.getAction()) {
			// 按下
...
//			mLoadMoreTextView.setText("没有更多数据了");
	...
	public void onScrollStateChanged(AbsListView view, int scrollState) {
		if (isAutoMore && lastItem == getAdapter().getCount()
				&& !loadingMoreState.equals(DListViewLoadingMore.LV_OVER)) {
			// 防止重复
			if (onRefreshLoadingMoreListener != null
